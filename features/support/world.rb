...
module Actionwords
  ...
  def i_tap_on_the_p1_button(string)
#      wait { $driver.find_element(:accessibility_id, "Let's Start").click }
      wait { $driver.find_element(:accessibility_id, string).click }
  end
 ...
 # end
  ...
  def i_should_see_the_main_dashboard_page
    puts "I should see the main dashboard page"
  end
end

def i_create_new_org(org_name)
  wait { $driver.find_element(:accessibility_id, 'Next').click }
  puts 'Create Org ' + org_name
  wait { $driver.find_element(:accessibility_id, 'AddCreateNewButton').click }
  wait { $driver.find_element(:accessibility_id, 'OrganisationName').send_keys(org_name) }
  wait { $driver.find_element(:accessibility_id, 'Create').click }
end
